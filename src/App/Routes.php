...
$app->group('/api/v1', function () use ($app) {
    ...
    $app->group('/files', function () use ($app) {
        /** Get all files */
        $app->get('', 'App\Controller\FileController:index');

        /** Upload file */
        $app->post('/upload', 'App\Controller\Note\GetAll');

        /** Update file by id */
        $app->patch('/[{id}]', 'App\Controller\Note\GetAll');

        /** Get one file data */
        $app->get('/[{id}]', 'App\Controller\Note\GetAll');

        /** Remove file from storage */
        $app->delete('/[{id}]', 'App\Controller\Note\GetAll');
    });
});
